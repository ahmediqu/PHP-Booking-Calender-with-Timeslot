<?php
  function build_calender($month,$year){
    ...
    while ($currentDay <= $numberOfDays) {
      ...
      if($dayname == 'saturday'){
        $calender .= "<td class='today  text-center'><h4>$currentDay</h4> <button class='btn btn-secondary btn-sm'>Holy Day</button></td>";
      }elseif($date<date('Y-m-d')){
        $calender .= "<td class='today text-center '><h4>$currentDay</h4> <button class='btn btn-danger btn-sm'>N/A</button></td>";
      }elseif(in_array($date, $bookings)){
        $calender .= "<td class='$today text-center'><h4>$currentDay</h4> <a href='book.php?date=".$date."' class='btn btn-primary btn-sm'>Already Booking</a></td>";
      }else{
        $calender .= "<td class='$today text-center'><h4>$currentDay</h4> <a href='book.php?date=".$date."' class='btn btn-success btn-sm'>Book</a></td>";
      }
      // if($dateToday == $date){
